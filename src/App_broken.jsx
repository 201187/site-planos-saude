...
import { ChevronLeft, ChevronRight, CheckCircle, Clock, Lightbulb, Star, MessageCircle } from "lucide-react";
import './App.css'
...
impo  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      sections.forEach((section) => {
        ...
        if (sectionTop < viewportHeight - 100) { // 100px from bottom of viewport
  ...
  }, []);  // Reset auto-scroll amount after manual interaction
      // This will make auto-scroll continue from the new position
      scrollAmount = targetScroll; // Update scrollAmount to continue auto-scroll from new position
    }
  };
      // Reset auto-scroll amount after manual interaction
      // This will make auto-scroll continue from the new position
  ...
  return (
    <div className="min-h-screen bg-white text-gray-800 p-6">
      ...
      {/* Hero Sec<section className="bg-blue-100 rounded-2xl shadow-md p-6 max-w-2xl mx-auto mb-10">2xl font-semibold mb-4 text-blue-800">Solicite sua cotação</h2>
      ...
      <section className="max-w-3xl mx-auto mb-16">
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 text-center">
          <li className="bg-blue-200 rounded-xl p-4 hover:bg-blue-300 transition-colors flex items-center justify-center gap-2"><CheckCircle size={20} className="text-green-600" /> Atendimento 100% personalizado</li>

          <li className="bg-blue-200 rounded-xl p-4 hover:bg-blue-300 transition-colors flex items-center justify-center gap-2"><Lightbulb size={20} className="text-green-600" /> Consultoria gratuita e sem compromisso</li>
          <li className="bg-blue-200 rounded-xl p-4 hover:bg-blue-300 transition-colors flex items-center justify-center gap-2"><Star size={20} className="text-green-600" /> As melhores operadoras do mercado</li>
          <li className="bg-blue-200 rounded-xl p-4 hover:bg-blue-300 transition-colors flex items-center justify-center gap-2"><MessageCircle size={20} className="text-green-600" /> Suporte direto via WhatsApp</li>
        </ul>
      </section>
      ...
      <section className="max-w-5xl mx-auto mb-16">
        ...
        <div className="relative">
          <div className="absolute top-1/2 left-0 transform -translate-y-1/2 z-10">
            <button onClick={() => scrollCarousel("left")} className="bg-blue-700 text-white p-2 rounded-full shadow hover:bg-blue-800 transition-colors">
              <ChevronLeft size={20} />
            </button>
          </div>
          <div className="absolute top-1/2 right-0 transform -translate-y-1/2 z-10">
            <button onClick={() => scrollCarousel("right")} className="bg-blue-700 text-white p-2 rounded-full shadow hover:bg-blue-800 transition-colors">
